...
# 读取数据
data <- read.csv("time_series.txt",header = TRUE, sep=',')

# 数据预处理
ts_data <- ts(data$Orders) # 转换为时间序列格式
# 拆分数据
train_ts <- ts_data[1:100]
test_ts <- ts_data[101:nrow(data)]

# 模型训练和校验
model <- auto.arima(train_ts,ic=c('aic'),trace=T) # 训练模型
summary(model) # 查看模型
in_ts_predict <- predict(model,(nrow(data)-100)) # 检验数据
